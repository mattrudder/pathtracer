...
fn metallic_scatter(r: Ray, point: Vector3, normal: Vector3, albedo: Vector3, roughness: f32) -> Option<Bounce> {
...
fn dielectric_scatter(r: Ray, point: Vector3, normal: Vector3, refractive_index: f32) -> Option<Bounce> {
...
impl Material {
    ...
    pub fn scatter(&self, r: Ray, point: Vector3, normal: Vector3) -> Option<Bounce> {
        match self {
            ...
            Material::Metallic { albedo, roughness } => metallic_scatter(r, point, normal, *albedo, *roughness),
            Material::Dielectric { refractive_index} => dielectric_scatter(r, point, normal, *refractive_index),
        }
    }
}
